...
# start_wavelength, end_wavelength = 400, 997
# spectra = spectra.loc[:, (spectra.columns >= start_wavelength) & (spectra.columns <= end_wavelength)]
...
SG_preprocessing = make_pipeline(
    ...
    LinearCorrection(),
    SavitzkyGolay(window_size=20, polynomial_order=1, derivate_order=0),
    StandardScaler(with_std=False)
    # MinMaxScaler()
)

# SG_spectra_preprocessed_d0 = SG_preprocessing.fit_transform(spectra_np)
SG_spectra_preprocessed_d0 = SG_preprocessing.fit_transform(spectra_np_log)
...
SG_spectra_preprocessed_d1 = SG_preprocessing.fit_transform(spectra_np)
...
SG_spectra_preprocessed_d2 = SG_preprocessing.fit_transform(spectra_np)



# get the wavenumbers after the range cut
# wavenumbers_cut = spectra.columns.to_numpy()
wavenumbers_cut = SG_preprocessing.named_steps['rangecut'].wavenumbers_

# plot the preprocessed spectra
plot_spectra(SG_spectra_preprocessed_d0, wavenumbers_cut, label_np, chartTitle="Savitzky-Golay derivate_order=0 Reflectance")
plot_spectra(SG_spectra_preprocessed_d1, wavenumbers_cut, label_np, chartTitle="Savitzky-Golay derivate_order=1 Reflectance")
plot_spectra(SG_spectra_preprocessed_d2, wavenumbers_cut, label_np, chartTitle="Savitzky-Golay derivate_order=2 Reflectance")

SG_spectra_preprocessed = np.hstack((
    SG_spectra_preprocessed_d0,
    SG_spectra_preprocessed_d1,
    SG_spectra_preprocessed_d2
))
''' change parameter '''
...
SG_label_pred = best_model.predict(X_test)
...
MF_label_pred = best_model.predict(X_test_m)
...
def plot_feature_importance(final_models, model_name, x_labels):
    ...
    plt.figure(figsize=(100, 46))
    plt.bar(x_labels, feature_importances, color='green')
    plt.title(f'{model_name} Feature Importance', fontsize=80)
    plt.xlabel('Wavelength (nm)')
    plt.ylabel('Importance Score', fontsize=60)
    plt.xticks(rotation=90, fontsize=10)  # Rotate x-tick labels for clarity
    ...
    sorted_x_labels = x_labels[sorted_indices]  # Corresponding sorted wavelengths
...
X_labels_d0 = np.array([f"{label}_d0" for label in x_labels])
X_labels_d1 = np.array([f"{label}_d1" for label in x_labels])
X_labels_d2 = np.array([f"{label}_d2" for label in x_labels])
X_labels_combined = np.concatenate((X_labels_d0, X_labels_d1, X_labels_d2))

for model_name in ['PLSR', 'SVR', 'Random Forest', 'ANN', 'KNN', 'XGBoost']:
    try:
        plot_feature_importance(final_models, model_name, X_labels_combined)
