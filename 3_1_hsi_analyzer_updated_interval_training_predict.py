...
label_file_path = r"E:/ufl/Aim/Aim_1/HSI_spectral_data_summary_HV_Updated.xlsx"
# label = pd.read_excel(label_file_path)
label = pd.read_excel(label_file_path)['Brix Level']
...
print(label.unique())
class_mapping = {'6-7.9 Brix': 0, '8-9.9 Brix': 1, '10-11.9 Brix': 2, '12-13.9 Brix': 3, '14-16 Brix': 4}
label = label.map(class_mapping)
label.describe()
...
grid_search_svc = GridSearchCV(svr, param_grid, cv=10, scoring='neg_mean_absolute_error')
...
grid_search_svc.fit(X_train, y_train_1d)

# Predict the Brix levels using the best SVR model
SG_label_pred_svc = grid_search_svc.best_estimator_.predict(X_train)

# Plot the predictions (similar to PLSR)
plt.scatter(y_train_1d, SG_label_pred_svc, color='blue')
plt.plot([0, 5], [0, 5], color='magenta')
...
# Evaluate the model
print("Best parameters for SVR: ", grid_search_svc.best_params_)
print(f"R²: {r2_score(y_train_1d, SG_label_pred_svc):.3f}")
print(f"MSE: {mean_squared_error(y_train_1d, SG_label_pred_svc):.3f}")
...
SG_label_pred_rf = rf.predict(X_train).round()
...
# #     # mse_scores = -mse_scores  # Convert negative MSE values to positive
# #     r2_scores = cross_val_score(best_model, X_test, y_test_1d, cv=cv, scoring='r2')
# #     mse_scores = cross_val_score(best_model, X_test, y_test_1d, cv=cv, scoring='neg_mean_squared_error')
# #     mse_scores = -mse_scores  # Convert negative MSE values to positive
...
# # feature_importances_rf = final_models['Random Forest'].feature_importances_

# # x_labels = np.arange(400, 999.4, 2.7)

# # # Plot the feature importances
# # plt.figure(figsize=(10, 6))
# # plt.bar(x_labels, feature_importances_rf, color='blue')
...
# # feature_importances_xgb = final_models['XGBoost'].feature_importances_

# # x_labels = np.arange(400, 400 + len(feature_importances_xgb))

# # # Plot the feature importances
# # plt.figure(figsize=(10, 6))
# # plt.bar(x_labels, feature_importances_xgb, color='green')
...
# # # Assuming final_models['XGBoost'].feature_importances_ is available
# # # Replace with actual feature names and importance values
# # feature_importances = final_models['XGBoost'].feature_importances_
